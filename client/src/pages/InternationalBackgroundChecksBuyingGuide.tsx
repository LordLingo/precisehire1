...
import {
  INTERNATIONAL_BACKGROUND_CHECKS_BUYING_GUIDE_MARKDOWN,
  INTERNATIONAL_BACKGROUND_CHECKS_BUYING_GUIDE_POST as post,
} from "@/content/international_background_checks_buying_guide_post";
import { resolveAuthor } from "@/content/authors";

export default function InternationalBackgroundChecksBuyingGuide() {
    ...
    <>
      ...
      <article className="container pb-20">
        <div className="mx-auto max-w-4xl prose prose-lg prose-precisehire max-w-none">
          <Streamdown>{INTERNATIONAL_BACKGROUND_CHECKS_BUYING_GUIDE_MARKDOWN}</Streamdown>
        </div>
        <div className="mx-auto max-w-4xl mt-12 rounded-2xl bg-[#0B1F3A] text-white p-6 sm:p-8 flex flex-col sm:flex-row gap-5 sm:items-center">
          <div className="flex-1">
            <p className="text-xs uppercase tracking-[0.16em] font-semibold text-[#E26C72]">International screening review</p>
            <h3 className="mt-2 font-display text-xl sm:text-2xl font-semibold leading-snug">Hiring across borders?</h3>
            <p className="mt-2 text-[14.5px] text-white/70 leading-relaxed">Send us the countries, job types, and screening package you need. We can map available searches, candidate requirements, expected turnaround, and workflow.</p>
          </div>
          <Link href="/talk-to-an-expert" className="btn-coral shrink-0 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[13.5px] font-semibold whitespace-nowrap">Review your countries</Link>
        </div>
      </article>
    </>
  );
