...
import {
  ATS_INTEGRATION_GUIDE_MARKDOWN,
  ATS_INTEGRATION_GUIDE_POST as post,
} from "@/content/ats_integration_guide_post";
import { resolveAuthor } from "@/content/authors";

export default function BackgroundCheckATSIntegrationGuide() {
    ...
    <>
      ...
      <article className="container pb-20">
        <div className="mx-auto max-w-4xl prose prose-lg prose-precisehire max-w-none">
          <Streamdown>{ATS_INTEGRATION_GUIDE_MARKDOWN}</Streamdown>
        </div>
        <div className="mx-auto max-w-4xl mt-12 rounded-2xl bg-[#0B1F3A] text-white p-6 sm:p-8 flex flex-col sm:flex-row gap-5 sm:items-center">
          <div className="flex-1">
            <p className="text-xs uppercase tracking-[0.16em] font-semibold text-[#E26C72]">Integration workflow review</p>
            <h3 className="mt-2 font-display text-xl sm:text-2xl font-semibold leading-snug">Does your ATS integration remove work or just move it?</h3>
            <p className="mt-2 text-[14.5px] text-white/70 leading-relaxed">We will review package rules, candidate consent, status mapping, security, billing, and implementation requirements with your team.</p>
          </div>
          <Link href="/talk-to-an-expert" className="btn-coral shrink-0 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[13.5px] font-semibold whitespace-nowrap">Talk to an integration specialist</Link>
        </div>
      </article>
    </>
  );
