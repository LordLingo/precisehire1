...
import {
  DOT_VS_NON_DOT_DRUG_TESTING_MARKDOWN,
  DOT_VS_NON_DOT_DRUG_TESTING_POST as post,
} from "@/content/dot_vs_non_dot_drug_testing_post";
import { resolveAuthor } from "@/content/authors";

export default function DOTVsNonDOTDrugTestingGuide() {
    ...
    <>
      ...
      <article className="container pb-20">
        <div className="mx-auto max-w-4xl prose prose-lg prose-precisehire max-w-none">
          <Streamdown>{DOT_VS_NON_DOT_DRUG_TESTING_MARKDOWN}</Streamdown>
        </div>
        <div className="mx-auto max-w-4xl mt-12 rounded-2xl bg-[#0B1F3A] text-white p-6 sm:p-8 flex flex-col sm:flex-row gap-5 sm:items-center">
          <div className="flex-1">
            <p className="text-xs uppercase tracking-[0.16em] font-semibold text-[#E26C72]">Drug-testing program review</p>
            <h3 className="mt-2 font-display text-xl sm:text-2xl font-semibold leading-snug">Are DOT and non-DOT orders separated automatically?</h3>
            <p className="mt-2 text-[14.5px] text-white/70 leading-relaxed">We will review position rules, testing panels, collection coverage, MRO workflow, Clearinghouse responsibilities, ATS status mapping, and total program pricing.</p>
          </div>
          <Link href="/talk-to-an-expert" className="btn-coral shrink-0 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[13.5px] font-semibold whitespace-nowrap">Talk to a testing specialist</Link>
        </div>
      </article>
    </>
  );
