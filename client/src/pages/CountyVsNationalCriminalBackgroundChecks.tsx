...
import {
  COUNTY_VS_NATIONAL_CRIMINAL_CHECKS_MARKDOWN,
  COUNTY_VS_NATIONAL_CRIMINAL_CHECKS_POST as post,
} from "@/content/county_vs_national_criminal_checks_post";
import { resolveAuthor } from "@/content/authors";

export default function CountyVsNationalCriminalBackgroundChecks() {
    ...
    <>
      ...
      <article className="container pb-20">
        <div className="mx-auto max-w-4xl prose prose-lg prose-precisehire max-w-none">
          <Streamdown>{COUNTY_VS_NATIONAL_CRIMINAL_CHECKS_MARKDOWN}</Streamdown>
        </div>
        <div className="mx-auto max-w-4xl mt-12 rounded-2xl bg-[#0B1F3A] text-white p-6 sm:p-8 flex flex-col sm:flex-row gap-5 sm:items-center">
          <div className="flex-1">
            <p className="text-xs uppercase tracking-[0.16em] font-semibold text-[#E26C72]">Criminal screening package review</p>
            <h3 className="mt-2 font-display text-xl sm:text-2xl font-semibold leading-snug">Not sure whether your current package has the right county and database coverage?</h3>
            <p className="mt-2 text-[14.5px] text-white/70 leading-relaxed">We can compare your current searches, verification workflow, turnaround, pass-through fees, and ATS setup side by side.</p>
          </div>
          <Link href="/talk-to-an-expert" className="btn-coral shrink-0 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[13.5px] font-semibold whitespace-nowrap">Compare your package</Link>
        </div>
      </article>
    </>
  );
