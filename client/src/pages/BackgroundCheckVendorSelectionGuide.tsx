...
import {
  BACKGROUND_CHECK_VENDOR_SELECTION_MARKDOWN,
  BACKGROUND_CHECK_VENDOR_SELECTION_POST as post,
} from "@/content/background_check_vendor_selection_post";
import { resolveAuthor } from "@/content/authors";

export default function BackgroundCheckVendorSelectionGuide() {
    ...
    <>
      ...
      <article className="container pb-20">
        <div className="mx-auto max-w-4xl prose prose-lg prose-precisehire max-w-none">
          <Streamdown>{BACKGROUND_CHECK_VENDOR_SELECTION_MARKDOWN}</Streamdown>
        </div>
        <div className="mx-auto max-w-4xl mt-12 rounded-2xl bg-[#0B1F3A] text-white p-6 sm:p-8 flex flex-col sm:flex-row gap-5 sm:items-center">
          <div className="flex-1">
            <p className="text-xs uppercase tracking-[0.16em] font-semibold text-[#E26C72]">Background check vendor comparison</p>
            <h3 className="mt-2 font-display text-xl sm:text-2xl font-semibold leading-snug">Want to compare your current screening provider side by side?</h3>
            <p className="mt-2 text-[14.5px] text-white/70 leading-relaxed">Send us your current package and invoice. We can compare search coverage, turnaround, integrations, support, and total cost.</p>
          </div>
          <Link href="/talk-to-an-expert" className="btn-coral shrink-0 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[13.5px] font-semibold whitespace-nowrap">Compare providers</Link>
        </div>
      </article>
    </>
  );
